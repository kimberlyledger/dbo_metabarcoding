...
okay, after comparing results from the two years and finding much greater taxa richness in 2023 and 2021, i wannt to try keeping all pcr reps seperate to see if the 1 vs 3 pcr reps is causing the difference... 

```{r}
taxon_table_1 <- asv_table %>%
  group_by(extraction_ID, replicate, project_year, project, collection_year, sample_type, taxon, taxonomic_level, species, genus, family, order, class) %>%
  summarize(tot_reads = sum(reads))
```
...
```{r}
#taxon_table_wide <- taxon_table_1[,c(1,6,13)] %>%     ## combined pcr reps
taxon_table_wide <- taxon_table_1[,c(1,2,7,14)] %>%    ## pcr reps seperate 
  mutate(tot_reads = as.integer(tot_reads)) %>%
  pivot_wider(names_from = taxon, values_from = tot_reads)

taxon_table_wide <- taxon_table_wide %>%
  unite(col = "full_ID", extraction_ID, replicate, sep = "_")

IDs <- taxon_table_wide$full_ID
...
which pcr reps are combined, visually, it looks like around 10,000 reads per sample was enough to saturate taxonomic diversity. 


XXXX - what is the total reads per sampling station (1 PCR rep per extraction for 2021; 3 combined PCR reps per extraction in 2023)
update - now all pcr reps are seperate for 2023
```{r}
reads <- taxon_table_1 %>%
  group_by(extraction_ID, replicate, project_year, sample_type) %>%
  summarize(reads = sum(tot_reads)) %>%
  arrange(desc(reads)) %>%
  unite(col = "full_ID", extraction_ID, replicate, sep = "_")

ggplot(reads, aes(x = reorder(full_ID, -reads), y = reads)) +
...
XXX - for any community analysis, i will remove any extractions with less than 1000 reads total from the dataset. 
update : for when PCR reps are kept separate, 
...
pcr_reps_lessthan_100 <- taxon_table_1 %>%
  group_by(extraction_ID, replicate, project_year, sample_type) %>%
  summarize(reads = sum(tot_reads)) %>%
  filter(reads < 100) %>%
  unite(col = "full_ID", extraction_ID, replicate, sep = "_", remove = F)

#extractions in bottles <1000 but not pcr rep <500
setdiff(bottles_lessthan_1000$extraction_ID, pcr_reps_lessthan_500$extraction_ID)

#extractions in pcr rep <500 but not bottles <1000 
setdiff(pcr_reps_lessthan_500$extraction_ID, bottles_lessthan_1000$extraction_ID)

## how many of the bottles do have more than 1000 reads combined after pcr reps under 500 are removed?? 
taxon_table_1 %>%
  unite(col = "full_ID", extraction_ID, replicate, sep = "_", remove = F) %>%
  filter(extraction_ID %in% pcr_reps_lessthan_500$extraction_ID) %>%
  filter(full_ID %in% pcr_reps_lessthan_500$full_ID) %>%
  group_by(extraction_ID, project_year, sample_type) %>%
  summarize(reads = sum(tot_reads)) %>%
  filter(reads > 999)
...
taxon_table_2 <- taxon_table_1 %>%
  ...
  unite(col = "full_ID", extraction_ID, replicate, sep = "_", remove = F) %>%
  filter(!full_ID %in% pcr_reps_lessthan_100$full_ID)
```
...
#write.csv(taxon_table_2, "/home/kimberly.ledger/dbo_metabarcoding/outputs/decontaminated_taxon_table_pcr_reps.csv")
```
...
mammalia_wide <- mammalia[,c(1,2,7,14)] %>%
  pivot_wider(names_from = taxon, values_from = tot_reads)
